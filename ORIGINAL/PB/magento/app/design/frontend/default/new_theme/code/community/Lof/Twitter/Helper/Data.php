 ...
 */
class Lof_Twitter_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	function get( $attributes = array() )
	{
		...
		{  	
			$pieces = array('lof_twitter', 'main_setting','advanded_parameter');
			$var = trim( $var );
			foreach($pieces as $piece){
				if(Mage::getStoreConfig("lof_twitter/$piece/$var")!=""){
					$data[$var] =  Mage::getStoreConfig("lof_twitter/$piece/$var");
				}
			}
        ...
        return Mage::getSingleton('lof_twitter/config')->getBaseMediaUrl() . $url;
    }
